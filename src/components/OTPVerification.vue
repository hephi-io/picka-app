...
import { testSavedAcc } from '@/utils';
...
const handleSubmit = async (event) => {
  ...
  try {
    ...
    let validCode = testSavedAcc.getTestToken();

    if (validCode === code) {
      ErrMessage.value = null;
      const data = axios.put(
        `${import.meta.env.VITE_API_BASE_URL}users/activate/${code}`,
      );
...
const moveToNext = (index) => {
  if (Activiation_code.value[index].length === 1 && index < Activiation_code.value.length - 1) {
  ...
  <div class="p-8">
    ...
    <main class="flex justify-center ">
      <section class="w-[36.3%] ">
        ...
        <form class="rounded-2xl md:border border-[#DFDFDF] p-6 " @submit="handleSubmit">
          ...
          <header class="mb-[60px]">
            ...
            <p class=" text-center outfit-normal font-normal text-base leading-[19.36px] text-[#26203B]">
              An OTP has been sent to the phone number linked to your account.
            </p>

          </header>

          <section class="h-[62px]  flex justify-between  items-center gap-2  mb-[60px]">
            <input v-for="(__, index) in Activiation_code" :key="index" v-model="Activiation_code[index]" type="text"
              maxlength="1" @input="moveToNext(index)" ref="inputs"
          ...
          <section class="flex justify-between mb-[60px]">
            ...
            <section>
              <button class="h-5 flex rounded-full px-2 py-1 bg-[#F4F4F4]">
                ...
                <section class="h-full flex items-center">
                  <span class="outfit-normal font-normal text-sm leading-[16.94px] text-[#26203B]">
                    Resend code
                  </span>
                </section>
            ...
            <section>
              ...
              <span class="outfit-normal font-normal text-[10px] leading-[12.1px]">in </span>
              <span class="outfit-semibold font-semibold text-[10px] leading-[12.1px] text-[#FA3105]">00.30s</span>
          ...
          <button type="submit"
            ...
            <Loader2 v-if="isSubmitting" class="w-5 h-5 mr-2 animate-spin text-white" />
            <span class="outfit-bold font-bold text-base leading-[19.36px] text-white">Verify account</span>
          </button>
    ...
    <footer class="h-[53px] mt-5 flex justify-between items-end">
      ...
      <section>
        <span class="outfit-normal font-normal text-sm leading-[21px] text-[#4F4D55]">©2024 Picka.ng</span>
      </section>
