...
const notSelectedToggleTabStyle =
  "scroll-snap-center box-border  flex-shrink-0 w-[284px] h-full bg-white rounded-lg mx-auto lg:w-full lg:h-[200px] rounded-lg border border-[#E4E7EC] p-4 mb-6";
const selectedToggleTabStyle =
  "scroll-snap-center box-border flex-shrink-0 w-[284px] h-full bg-white rounded-lg mx-auto lg:w-full lg:h-[200px] rounded-lg border border-[#F1C49B] p-4 mb-6 toggle";
...
const selectToggleTab = async (shipment) => {
  ...
  try {
    const { data: activitiesResponse } = await getShipmentActivities(
      shipment.id
    );
...
const selectedHorizontalTabStyle =
  "w-full md:w-[181.99px] rounded px-3 py-[6px] tabs-trigger";
const notSelectedHorizontalTabStyle =
  "w-full md:w-[181.99px] rounded px-2 py-[6px]";
...
<template>
  <div>
    <header class="hidden md:block py-3 mb-9">
      <h1
        class="font-semibold text-2xl leading-6 tracking-[-0.03em] text-[#060E1F]"
      >
        Tracking
      </h1>
    </header>
    <section class="gap-x-6 lg:flex">
      <section class="lg:flex-1">
        ...
        <section
          class="flex gap-x-2 justify-between items-center mb-4 md:hidden"
        >
          <header class="py-3">
            <h1
              class="font-semibold text-2xl leading-6 tracking-[-0.03em] text-[#060E1F]"
        ...
        >
          <ToggleGroupItem
            :value="shipment.id"
            :class="[
              selectedToggleTab === shipment.id
                ? selectedToggleTabStyle
                : notSelectedToggleTabStyle,
            ]"
            v-for="shipment in shipments"
            :key="shipment.id"
            @click="selectToggleTab(shipment)"
          >
      ...
      <section
        class="flex-grow rounded-xl border border-[#E4E7EC] shadow-sm shadow-[#1018280D]"
      >
        <Tabs :default-value="horizontalTabValue1">
          <div class="px-2 md:px-6 py-5">
            <section class="flex justify-between items-center mb-4">
              ...
              >
                <span
                  class="font-medium text-sm leading-[18px] text-center text-[#309BFF]"
                >
                  In transit
                </span>
              </Badge>
            ...
            <TabsList
              class="w-full md:w-[373.99px] rounded-lg p-[5px] bg-[#F5F7FA]"
            >
              <TabsTrigger
                :value="horizontalTabValue1"
                :class="
                  selectedHorizontalTab === horizontalTabValue1
                    ? selectedHorizontalTabStyle
                    : notSelectedHorizontalTabStyle
                "
              ...
              <TabsTrigger
                ...
                :class="
                  selectedHorizontalTab === horizontalTabValue2
                    ? selectedHorizontalTabStyle
                    : notSelectedHorizontalTabStyle
                "
          ...
          <TabsContent value="status-based-tracking">
            <div class="h-[636px] relative lg:min-w-[34rem]">
              <div class="px-4 py-[52px] border-t">
                <Stepper
                  class="flex lg:items-start flex-col gap-x-0 lg:flex-row"
                >
                  <StepperItem
                    v-for="(activity, index) in shipmentActivities"
                    :key="activity.id"
                    :step="index + 1"
                  ...
                  >
                    ...
                    >
                      <div
                        class="flex flex-col lg:flex-row lg:w-full justify-start"
                      >
                        <StepperIndicator
                          :class="[
                        ...
                        >
                          <div
                            class="w-full h-full flex justify-center items-center rounded-full bg-white"
                          >
                            <order-confirmed-icon
                              v-if="activity.status === 'pending'"
                            />
                            <picked-up-icon
                              v-else-if="activity.status === 'shipping'"
                            />
                            <in-transit-icon
                              v-else-if="activity.status === 'in transit'"
                            />
                            <package-delivered
                              v-else-if="activity.status === 'delivered'"
                            />
                      ...
                      <div>
                        ...
                        <StepperDescription class="w-full lg:w-[83%]">
                          <section
                            class="font-normal text-xs leading-5 text-[#475467] text-left"
                          >
                            {{ getDescription(activity.status) }}
                          </section>
                          <section
                            class="mt-1 font-normal text-xs leading-5 text-[#37404D80] text-left"
                          >
              ...
              <div class="hidden lg:block">
                <info-tab-component
                  ...
                  class="border border-[#E4E7EC] info-tab-shadow"
                />
              </div>
            </div>
          </TabsContent>
