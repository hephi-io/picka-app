...
import TabsList from "@/components/shared/p-tabs/tabs-list.vue";
import TabsTrigger from "@/components/shared/p-tabs/tabs-trigger.vue";
import LoginSlider from "./components/login-slider.vue";
...
<template>
  <div class="h-[100vh] w-full">
    <div class="flex h-[92dvh]">
      <section class="hidden md:block w-1/2 pt-8 pl-8">
        <div class="bg-gray-200 h-full rounded-[20px]">
          <login-slider />
        </div>
      </section>

      <!-- Mobile view -->
      <section class="w-full md:w-1/2 flex justify-center overflow-auto">
        <div class="w-[342px] xl:w-[500px] pt-[18dvh]">
          <div class="mb-8">
            <!-- Logo -->
            <div class="mb-9">
              <svg width="91" height="33" viewBox="0 0 91 33" fill="none">
                <path
            ...
            <div class="mt-5">
              <motion.div
                ...
                :key="activeTab"
              >
                <component :is="activeTabMap[activeTab]" />
              </motion.div>
            </div>
          </div>
    ...
    <footer class="hidden lg:flex lg:justify-between lg:pt-8 px-8">
      <span class="font-normal text-sm leading-[21px] text-[#4F4D55]">
        ©{{ new Date().getFullYear() }} picka.delivery
      </span>
